import EZWCompression
import getopt
import glob
from imports.math_tools import byte_size
from compression.compression_algos import (
    EZW_compression,
    EZW_decompression,
...
from imports.math_tools import psnr 
...
def PlotPhi2D_wavelets(kept_percentage, H5conf, H5rprofP, H5Phi2D, outputDir):
    ...
    # Martin's wavelet compression algorithm #
    _, Phi2D_rtheta = wavelet_nDcompression(
        Phi2D_rtheta,
        pywt.Wavelet("bior4.4"),
        kept_percentage,
        compression_method="deflate",
        reconstruction=True,
    )
    # Necessary to recover the good sizes
    Phi2D_rtheta = Phi2D_rtheta[:-1, :]
    print(
        "wavelets percent + deflate compression rate = ",
        byte_size(Phi2D_rtheta) / byte_size(_),
    )
    # -------------------------------------- #
    ...
    ax1 = fig.add_subplot(1, 1, 1)
    mpp.suptitle(
        r"$\Phi-\Phi_{{00}}$ at time = ${}/\omega_c$ with wavelets at {}%".format(
            timePhi2D, kept_percentage * 100
        ),
        size=18,
    )
    p1 = ax1.pcolormesh(
    ...
    figName = outputDir + "/Phi2Drtheta_wavelets_t" + str(round(timePhi2D)) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName)
    mpp.ion()


def PlotPhi2D_zfp(bpd_rate, H5conf, H5rprofP, H5Phi2D, outputDir):
    ...
    timePhi2D = H5Phi2D.time_diag[itimeDiag][0]
    Phi2D_rtheta = H5Phi2D.Phirth[itimeDiag, :, :]
    print(byte_size(Phi2D_rtheta))

    # zfp Compression #
    comp = zfp_compression(Phi2D_rtheta, bpd_rate)
    Phi2D_rtheta = zfp_decompression(Phi2D_rtheta, comp, bpd_rate)
    print("zfp compression rate = ", 64 / bpd_rate)
    # --------------- #
    ...
    ax1 = fig.add_subplot(1, 1, 1)
    mpp.suptitle(
        r"$\Phi-\Phi_{{00}}$ at time = ${}/\omega_c$ with lossy zfp at {} bpd".format(
            timePhi2D, bpd_rate
        ),
        size=18,
    )
    ...
    figName = outputDir + "/Phi2Drtheta_zfp_t" + str(round(timePhi2D)) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName)
    mpp.ion()


def PlotPhi2D_EZW(n_passes, H5conf, H5rprofP, H5Phi2D, outputDir):
    ...
    print(Phi2D_rtheta.shape)

    #            My EZW Compression        #
    wavelet = pywt.Wavelet("bior4.4")
    # By now we need renormalization
    renorm = 1 / np.min(np.abs(Phi2D_rtheta[np.nonzero(Phi2D_rtheta)]))
    start_time = time.time()
    encoder = EZW_compression(renorm * Phi2D_rtheta, wavelet, n_passes)
    print("took : {} s".format(time.time() - start_time))
    decoder = EZW_decompression(Phi2D_rtheta, wavelet, encoder, n_passes)
    rec_Phi2D_rtheta = decoder.getReconstruction() / renorm
    rec_Phi2D_rtheta = rec_Phi2D_rtheta[:-1, :]
    print("EZW compression rate : ", byte_size(Phi2D_rtheta) / len(encoder))
    print("psnr = ", psnr(Phi2D_rtheta, rec_Phi2D_rtheta))
    # -------------------------------------- #

    Phi2D_rtheta = rec_Phi2D_rtheta
    ...
    ax1 = fig.add_subplot(1, 1, 1)
    mpp.suptitle(
        r"$\Phi-\Phi_{{00}}$ at time = ${}/\omega_c$ with lossy EZW with {} passes".format(
            timePhi2D, n_passes
        ),
        size=18,
    )
    ...
    figName = outputDir + "/Phi2Drtheta_EZW_t" + str(round(timePhi2D)) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName)
    mpp.ion()
...
def PlotPhi2Dmostunstable_wavelets(kept_percentage, H5conf, H5Phi2D, outputDir):
    """
    Plot the time evolution of the eight most 
    unstable modes + (0,0) + (1,0)
    """
    ...
    # Martin's wavelet compression #

    _, Phi2D_thphi = wavelet_nDcompression(
        Phi2D_thphi, pywt.Wavelet("bior4.4"), kept_percentage, reconstruction=True
    )
    comp_rate = byte_size(Phi2D_thphi) / byte_size(_)
    print("wavelet fourier modes compression rate : ", comp_rate)
    # ------------------------------#
    ...
    mpp.ylabel(r"$abs(\Phi_{mn})(r)$")
    mpp.legend(loc=4)
    figName = outputDir + "/Fig_mn_mostunstable_wavelets_t" + str(timeEnd) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName, transparent=False, format="png")
    mpp.ion()


def PlotPhi2Dmostunstable_zfp(bpd_rate, H5conf, H5Phi2D, outputDir):
    """
    Plot the time evolution of the eight most 
    unstable modes + (0,0) + (1,0)
    """
    ...
    # zfp compression #
    comp = zfp_compression(Phi2D_thphi, bpd_rate)
    Phi2D_thphi = zfp_decompression(Phi2D_thphi, comp, bpd_rate)
    print("zfp fourier modes compression rate : {}".format(64 / bpd_rate))
    # --------------- #
    ...
    mpp.ylabel(r"$abs(\Phi_{mn})(r)$")
    mpp.legend(loc=4)
    figName = outputDir + "/Fig_mn_mostunstable_zfp_t" + str(timeEnd) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName, transparent=False, format="png")
    mpp.ion()


def PlotPhi2Dmostunstable_EZW(n_passes, H5conf, H5Phi2D, outputDir):
    """
    Plot the time evolution of the eight most 
    unstable modes + (0,0) + (1,0)
    """
    ...
    wavelet = pywt.Wavelet("bior4.4")
    start_time = time.time()
    renorm = 1 / np.min(np.abs(Phi2D_thphi[np.nonzero(Phi2D_thphi)]))
    encoder = EZW_compression(renorm * Phi2D_thphi, wavelet, n_passes)
    print("time : {} s".format(time.time() - start_time))
    decoder = EZW_decompression(Phi2D_thphi, wavelet, encoder, n_passes)
    Phi2D_thphi = decoder.getReconstruction() / renorm
    print("EZW compression rate :", byte_size(Phi2D_thphi) / len(encoder))
    # print(Phi2D_thphi.shape)
    Phi2D_thphi = Phi2D_thphi[:, :-1, :-1]
    # --------------- #
    ...
    mpp.ylabel(r"$abs(\Phi_{mn})(r)$")
    mpp.legend(loc=4)
    figName = outputDir + "/Fig_mn_mostunstable_EZW_t" + str(timeEnd) + ".png"
    print("Save figure:", figName)
    mpp.savefig(figName, transparent=False, format="png")
    mpp.ion()
...
if __name__ == "__main__":
    ...
    Phi2dFileNames = DirSp0 + "Phi2D/Phi2D_d*.h5"
    ...
    rprofpSp0FileNames = DirSp0 + "rprof/rprof_part_d*.h5"
    ...
    # Plot Phi2D
    outputDir = "/local/home/mc271598/Bureau/code/dask_wavelets/GYSELA_figures"
